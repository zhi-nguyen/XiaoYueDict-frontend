...
| Metric | Value |
|---|---|
| **Source files** | **134 files** |
| **Lines of code** | **20,281 LOC** |
| TypeScript (.ts) | 63 files · 5,222 LOC |
| React TSX (.tsx) | 70 files · 14,806 LOC |
| CSS | 1 files · 253 LOC |
| Pages (App Router) | 10 routes |
| Components | 29 (21 top-level + 8 subdirectories) |
| Custom Hooks | 10 |
| Zustand Stores | 9 |
| API Route Handlers | 12 BFF proxy routes |
| Total Commits | 62 |
...
- **Modals**: `AlertModal`, `ConfirmModal`, `ReportModal`, `PaymentQRModal`
- **Cards**: `WordCardZh`, `WordCardEn`, `TranslationCard`
...
See [`src/lib/api/proxy.ts`](src/lib/api/proxy.ts) for details.
...
|---|---|---|
| `useSmartQueue` | 429 | Task status poller with exponential backoff |
| `useWebSocket` | 252 | WebSocket interface with automatic reconnection |
| `useStudySearch` | 211 | Debounced searching wrapper supporting multiple dictionary formats |
| `useHanziDetails` | 216 | High-performance stroke data fetching and cache middleware |
| `useHanVietSentence` | 189 | Character-by-character Sino-Vietnamese transcription helper |
| `useAudioRecording` | 170 | Low-overhead MediaRecorder state and compression |
| `usePronunciationScorer` | 93 | Unified scoring and evaluation coordinator |
| `useNotificationWebSocket` | 76 | Isolated socket channel strictly for push notifications |
| `useSpellCheck` | 57 | Real-time browser-based spelling dictionary helper |
| `useDebounce` | 10 | Generic debouncer |
...
<sub>See also: [Backend README](../core_django/README.md)</sub>
